...
class Orbital:
    ...
    def get_last_an_time(self, utc_time):
        ...
        def an_time_func(minutes: float) -> float:
        ...
        def find_bracket(func, start_min, step_min, max_back_min):
            ...
            while abs(t_low) <= max_back_min:
                ...
                t_high = t_low
                t_low -= step_min
            raise ValueError("Could not find suitable bracket for Ascending Node.")
        ...
        root_min = _get_root(an_time_func, t_low_min, t_high_min, tol=0.0001)

        t_an = time_ref + np.timedelta64(int(root_min * 60), "s")

        _, vel = self.get_position(t_an, normalize=False)
        if vel[2] < 0:
            t_an -= np.timedelta64(int(orbit_period_min / 2.0 * 60), "s")
